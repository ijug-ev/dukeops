...
**In this document you will find a lot of information on how you can contribute to *DukeOps*.**
...
- [Quickstart for Local Development](#quickstart-for-local-development)
    ...
    - [Run DukeOps](#run-dukeops)
    - [Open in Browser](#open-in-browser)
    - [Stop DukeOps](#stop-dukeops)
...
To find possible tasks for your first contribution to *DukeOps*, we tagged some of the hopefully easier to solve issues as [good first issue](https://github.com/ijug-ev/dukeops/labels/good%20first%20issue).

If you prefer to meet people in real life to contribute to *DukeOps* together, we recommend to visit a [Hackergarten](https://www.hackergarten.net/) event. *DukeOps* is often selected as a contribution target in [Lucerne](https://www.meetup.com/hackergarten-luzern/), [Zurich](https://www.meetup.com/hackergarten-zurich/), and at the [CyberLand Open Source Camp](https://cyberland.ijug.eu/).

Please join our developer community using our [Matrix chat](#matrix-chat) to get support and help for contributing to *DukeOps*.
...
This guide helps you get *DukeOps* up and running on your local machine for development purposes.
...
1. Fork the [DukeOps repository](https://github.com/ijug-ev/dukeops) on GitHub.
2. Clone your fork locally:

   ```bash
   git clone https://github.com/YOUR_USERNAME/dukeops.git
   cd dukeops
   ```
...
*DukeOps* needs a database and a mail server to run. For local development, you can start both using the provided `docker-compose.yml` file in the `dev-tools` directory:
...
- MariaDB on Port 3306 with persistent data in `mariadb-data`.  
  The database user will be `dukeops` with the password `dukeops`. The database name is also `dukeops`.
...
*DukeOps* is configured using environment variables. At minimum, the following must be set:
...
DUKEOPS_DB_USER=dukeops
DUKEOPS_DB_PASS=dukeops
DUKEOPS_MAIL_HOST=localhost
DUKEOPS_MAIL_PORT=1025
```
...
You can run *DukeOps* either via your IDE or the command line. On the command line, use the Maven Wrapper:

```bash
./mvnw
```

This runs the `eu.ijug.dukeops.Application` main class.
...
|:---------------------------------------------|:-----------------------------------------------------------|
| `src/main/frontend/themes/dukeops/`          | Base directory for default *DukeOps* theme                 |
...
| `src/main/java/eu/ijug/dukeops/`             | Server-side source directory                               |
...
*DukeOps* follows the Boundary Control Entity (BCE) architecture pattern. This pattern divides the application into three main layers:
...
| Directory                               | Description                                           |
|:----------------------------------------|:------------------------------------------------------|
| `src/main/java/eu/ijug/dukeops/domain/` | Code for all business domains (BCE architecture)      |
...
| Directory                                         | Description                                 |
|:--------------------------------------------------|:--------------------------------------------|
| `src/main/java/eu/ijug/dukeops/domain/community/` | Subpackage for the "Community" domain       |
...
> [!IMPORTANT]  
> *DukeOps* uses the ICU message format for all translation files. This allows for advanced formatting options, including pluralization and named parameters. ICU stands for "International Components for Unicode".
...
As the key for the translations, we use a dot-separated hierarchical structure that reflects the package structure without the leading `eu.ijug.dukeops` and include the class name. This makes it easier to locate and manage translations.
...
*DukeOps* uses [Maven](https://maven.apache.org/) to build the project. Please use standard Maven commands to build what you need:
...
> [!TIP]
> There is an [official Docker image](https://hub.docker.com/repository/docker/ijug-ev/dukeops/general) you can use to run *DukeOps*.

To build the dockerized version of *DukeOps* yourself, run:

```
docker build . -t dukeops:latest
```

Once the Docker image is correctly built, you can test it locally using:

```
docker run -p 8080:8080 dukeops:latest
```
